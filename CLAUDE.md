...
**Rate limiting is in-memory and per-process** (`rate_limit.py`) — it does not persist across restarts/deploys and would not be shared if the backend ever scaled to more than one instance. The visitor is identified by `_ip_cliente()`, which reads the **last** entry of `X-Forwarded-For`: behind Railway's proxy `request.client.host` is not the visitor and varied per request, which silently disabled the limiter entirely in production. The last entry is the one the trusted proxy appends, so unlike the first it cannot be spoofed by the caller — but that assumes exactly one proxy hop. If a CDN is ever put in front, this needs revisiting or every visitor will collapse into a single bucket.
