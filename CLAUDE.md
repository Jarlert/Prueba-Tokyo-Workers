...
**Every order card is labelled `(Delivery)` or `(Pickup)` next to the customer name**, and the Finalizado card additionally repeats the name and phone (the other columns already show the order text). The label is decided by the *same* rule as the daily counter — `tipo_entrega` containing `pickup` or `retiro` — so a card's label can never disagree with its `#N`. The Finalizado column keeps a second, wider `esDelivery` (which also accepts a `Servicio de Delivery` line in the order text) purely for the pay-the-rider button; the two are deliberately not the same test.
...
**`precio_delivery` and `paga_con` are captured at different steps, on purpose.** The delivery quote (`pedirPrecioDelivery` in `app.js`, resolving to `{ precio }`) leads with a one-tap **standard price button** (`PRECIO_DELIVERY_ESTANDAR`, currently $2 — the constant drives both the button label and the value) and keeps a free "otro monto" field below it for the far addresses. **"¿Con cuánto paga?" lives one step later**, in the Validar Pago modal (`pedirComprobantePago`, resolving to `{ referencia, imagen, pagaCon }`), because quoting the delivery is what *sends* the customer the cobro message — the cashier only learns which bill they're bringing after that reply. It still lands before `notificar-despacho`, which is what the rider group reads. The field only appears for cash orders, where the reference field is hidden anyway.

Both travel to `/actualizar-estado`, which merges whatever keys it receives. That merge is why `ejecutarActualizacion` sends `paga_con` **only when `datosPago` carries a non-empty one**: it also runs on the Finalizado transition with `datosPago = null`, and an unconditional `paga_con: null` there would blank the value in the same instant the rider notice needs it.
